...
[[ -s "$BUILD_DIR/qualification-proof.json" ]] || { echo "ERROR: qualification-proof.json missing" >&2; exit 5; }
...
[[ -s "$BUILD_DIR/qualification/final-artifact-countercounteraudit-report.json" ]] || { echo "ERROR: final-artifact countercounteraudit report missing" >&2; exit 5; }

# Strict verification recomputes report and Web-payload hashes before any copy.
...
python3 - "$BUILD_DIR/build-info.json" "$BUILD_DIR/qualification-proof.json" \
  "$BUILD_DIR/qualification/final-artifact-countercounteraudit-report.json" \
  "$SOURCE_SHA" "$PRODUCT_REVISION" "$FULL_QUALIFICATION" <<'PY'
...
checks = [
    ...
    (proof.get("final_countercounteraudit_passed") is True, "qualification proof lacks final-artifact countercounteraudit success"),
    (proof.get("final_countercounteraudit_report_sha256") == final_hash, "final-artifact report hash mismatch"),
    (final.get("passed") is True, "final-artifact report is not passing"),
    (final.get("source_commit") == source_sha, "final-artifact report source commit mismatch"),
    (final.get("revision") == revision, "final-artifact report revision mismatch"),
]
