...
func _init() -> void:
	...
	if int(audio_contract.get("mix_rate", 0)) != 22050: errors.append("RC7 audio must synthesize at 22.05 kHz")
	if int(audio_contract.get("biome_motifs", 0)) != 5: errors.append("RC7 audio must expose five biome motifs")
	if int(audio_contract.get("warning_families", 0)) != 4: errors.append("RC7 audio must expose four warning families")
	if bool(audio_contract.get("external_samples", true)): errors.append("RC7 production audio must remain first-party synthesis")
	...
	if warning_hashes.size() != WARNING_IDS.size(): errors.append("Warning SFX families must be data-distinct")

	var fairness_report: Dictionary = FairnessDirectorScript.new().call("audit_contract")
	if float(fairness_report.get("room_grace", 0.0)) < 0.5: errors.append("Room grace below RC7 floor")
	if float(fairness_report.get("spawn_clearance", 0.0)) < 140.0: errors.append("Hostile spawn clearance too small")
	var evolution_report: Dictionary = EvolutionDirectorScript.new().call("audit_contract")
	if int(evolution_report.get("options", 0)) != 25: errors.append("RC7 must expose 25 guardian adaptations")
	...
	if main_scene == null:
		...
		if instance.has_method("audit_masterpiece_contract"):
			...
			for flag in ["semantic_warning_audio","room_entry_grace","staggered_enemy_materialization","safe_state_asset_prewarm","biome_pool_cache_retention","shallow_startup_deep_release_audit","masterpiece_asset_registry","pixel_finish","guardian_adaptation_choices","adaptation_suspend_safe","run_only_lineage_evolution","profile_schema_preseed"]:
				if not bool(runtime_report.get(flag, false)): errors.append("RC7 quality flag missing: " + flag)
		else: errors.append("RC7 runtime quality audit method missing")
		if instance.has_method("audit_godmode_contract"):
			...
			for flag in ["deterministic_floor_graph","rng_seed_and_state_restored","ending_choice_suspend_safe","serpent_resolution_choice"]:
				if not bool(inherited_report.get(flag, false)): errors.append("Inherited RC6 guarantee missing: " + flag)
		else: errors.append("Inherited RC6 audit contract unavailable")
		instance.free()

	var report := {"product_revision":"0.6.1-rc7","audio":audio_contract,"music_unique":music_hashes.size(),"ambience_unique":ambience_hashes.size(),"warning_unique":warning_hashes.size(),"fairness":fairness_report,"evolutions":evolution_report,"runtime":runtime_report,"inherited":inherited_report,"errors":errors,"passed":errors.is_empty()}
