...
const AUTHORED_ART_REVISION: String = "0.6.4-art4"
const PRODUCT_REVISION: String = "0.6.4-authored-art4"
const ART4_SPRITE_FORGE_PATH: String = "res://scripts/v8/procedural_sprite_forge_art4.gd"
const ART4_GENOME_DIRECTOR_PATH: String = "res://scripts/v8/enemy_genome_director_art4.gd"
const ART4_WORLD_DIRECTOR_PATH: String = "res://scripts/v8/procedural_world_director_art4.gd"

# The parent animation runtime owns Art4 component installation in _ready().
# Keeping a second Art3 pre-install here made the release root internally
# contradictory and allowed diagnostics to report an obsolete art stack.

func audit_release_binding_contract() -> Dictionary:
	return {
		"product_revision": PRODUCT_REVISION,
		"authored_art_revision": AUTHORED_ART_REVISION,
		"release_root": true,
		"sprite_forge_path": ART4_SPRITE_FORGE_PATH,
		"genome_director_path": ART4_GENOME_DIRECTOR_PATH,
		"world_director_path": ART4_WORLD_DIRECTOR_PATH,
		"parent_installs_art4": true,
		"legacy_art3_preinstall": false,
	}
...
func restore_suspended_run() -> void:
	...
	if not player.is_empty():
		player["v8_visual_genome"] = genome_director.call(
			"player_genome",
			entropy.call("fork", "canonical_restore"),
			String(player.get("id", "adam")),
			biome_index
		)
...
func update_boss(enemy: Dictionary, direction: Vector2, distance: float, delta: float) -> Vector2:
	if not bool(enemy.get("visual_pending", false)) and float(enemy.get("boss_windup", 0.0)) <= 0.0 and float(enemy.get("cooldown", 0.0)) <= 0.0:
		...
		var local_rng: RandomNumberGenerator = entropy.call(
			"fork",
			"boss_pattern:%s:%d" % [String(enemy.get("visual_key", enemy.get("id", "boss"))), cycle]
		)
...
func _play_trait_warning(role: String) -> void:
	...
	if role == "ranged":
		warning = "warning_aimed"
	elif role in ["caster", "radial", "orbiter"]:
		warning = "warning_radial"
...
func _release_entropy_trait_action(index: int) -> void:
	...
	match behavior_trait:
		"burst":
			for angle_variant: float in [-0.12, 0.0, 0.12]:
				enemy_shoot(origin, target.rotated(angle_variant), 380.0 * float(genome.get("projectile_speed_mult", 1.0)), 1.0)
		"marksman":
			enemy_shoot(origin, target, 520.0 * float(genome.get("projectile_speed_mult", 1.0)), 1.2)
		"suppression":
			for angle_variant: float in [-0.42, -0.21, 0.0, 0.21, 0.42]:
				enemy_shoot(origin, target.rotated(angle_variant), 305.0, 0.85)
		"scatter":
			for angle_variant: float in [-0.54, -0.36, -0.18, 0.0, 0.18, 0.36, 0.54]:
				enemy_shoot(origin, target.rotated(angle_variant), 280.0, 0.75)
		"ritual", "nova", "ring":
			var count: int = 8 if behavior_trait != "nova" else 12
			var phase: float = float(genome.get("phase_offset", 0.0)) + visual_clock * 0.11
			for shot: int in range(count):
				enemy_shoot(origin, Vector2.RIGHT.rotated(TAU * float(shot) / float(count) + phase), 275.0, 0.9)
		"seeker":
			for angle_variant: float in [-0.22, 0.0, 0.22]:
				enemy_shoot(origin, target.rotated(angle_variant), 340.0, 0.9)
		"zone", "minefield":
			for shot: int in range(6):
		...
		"flanker", "feint", "blink", "dashshot":
			...
			if behavior_trait == "dashshot":
				enemy_shoot(origin, target, 390.0, 0.9)
		"ram", "juggernaut", "shockwave", "breach":
			...
			if behavior_trait == "shockwave":
				for shot: int in range(6):
					enemy_shoot(origin, Vector2.RIGHT.rotated(TAU * float(shot) / 6.0), 190.0, 0.75)
		"orbit", "satellite", "spiral", "harrier":
			var count: int = 6 if behavior_trait != "spiral" else 9
			for shot: int in range(count):
				enemy_shoot(origin, Vector2.RIGHT.rotated(TAU * float(shot) / float(count) + visual_clock * 0.24), 250.0, 0.8)
		"ripper", "stalker", "leaper", "bloodrush":
			enemy["v8_burst_dir"] = target
			enemy["v8_burst_time"] = 0.20 if behavior_trait != "bloodrush" else 0.32
		_:
			if role in ["ranged", "caster", "radial", "orbiter"]:
				enemy_shoot(origin, target, 320.0, 0.85)
...
func get_v6_diagnostics() -> Dictionary:
	...
	report["art4_sprite_forge"] = true
	report["art4_genome_director"] = true
	report["art4_world_director"] = true
...
func audit_godmode_contract() -> Dictionary:
	...
	report["stochastic_guardian_pattern_order"] = true
	report["hero_identity_locked"] = true
	report["release_art4_binding"] = true
	return report
