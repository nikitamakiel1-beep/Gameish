...
func utility_texture(id: String) -> Texture2D:
	var valid := ["pickups", "relics", "projectiles", "effects", "hud_panel", "menu_panel", "joystick_base", "joystick_thumb", "touch_dash", "touch_interact", "touch_pause"]
	if id not in valid:
...
func validate_contract() -> Dictionary:
	...
	for id in expected.keys():
		if check_size(utility_texture(String(id)), Vector2i(expected[id]), "utility:" + String(id), errors): decoded += 1
	for id in BIOME_IDS:
