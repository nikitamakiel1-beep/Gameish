...
func build_pattern(boss_id: String, stage: int, pattern_index: int, target_direction: Vector2, phase: float) -> Dictionary:
	...
	var pattern: int = posmod(pattern_index, 3)
...
func _watcher_engine(stage: int, pattern: int, direction: Vector2, phase: float) -> Dictionary:
	if pattern == 0:
		return _result(_radial_shots(12 + stage * 4, 245.0 + float(stage) * 28.0, phase * 0.12), 0.58, maxf(0.72, 1.18 - float(stage) * 0.08), 0.48)
	if pattern == 1:
		return _result(_fan_shots(direction, [-0.34, -0.17, 0.0, 0.17, 0.34], 405.0 + float(stage) * 32.0), 0.44, 0.94, 0.62)
	var shots: Array = []
	for spoke in range(8):
		var spoke_direction: Vector2 = Vector2.RIGHT.rotated(TAU * float(spoke) / 8.0 + phase * 0.22)
...
func _first_nephilim(stage: int, pattern: int, direction: Vector2, phase: float) -> Dictionary:
	if pattern == 0:
		var charge_result: Dictionary = _result([], 0.72, 1.34, 0.08)
		charge_result["charge_duration"] = 0.34 + float(stage) * 0.05
		charge_result["charge_multiplier"] = 4.1 + float(stage) * 0.45
		return charge_result
	if pattern == 1:
		return _result(_radial_shots(14 + stage * 4, 258.0 + float(stage) * 24.0, phase * 0.06), 0.62, 1.20, 0.34)
	return _result(_fan_shots(direction, [-0.42, -0.21, 0.0, 0.21, 0.42], 430.0 + float(stage) * 34.0), 0.52, 1.04, 0.56)
...
func _tower_enoch(stage: int, pattern: int, direction: Vector2, phase: float) -> Dictionary:
	if pattern == 0:
		var crossfire: Array = []
		for axis in range(8):
			var base: Vector2 = Vector2.RIGHT.rotated(TAU * float(axis) / 8.0)
	...
	if pattern == 1:
		var spiral: Array = []
		var count: int = 12 + stage * 4
		for index in range(count):
			var angle: float = TAU * float(index) / float(count) + phase * 0.28
			spiral.append(_shot(Vector2.RIGHT.rotated(angle), 250.0 + float(index % 3) * 28.0, 1.0))
		return _result(spiral, 0.52, 1.02, 0.22)
	var summon_result: Dictionary = _result(_fan_shots(direction, [-0.48, -0.24, 0.0, 0.24, 0.48], 390.0), 0.78, 1.42, 0.06)
...
func _serpent_interface(stage: int, pattern: int, direction: Vector2, phase: float) -> Dictionary:
	if pattern == 0:
		var spiral: Array = []
		var count: int = 16 + stage * 4
		for index in range(count):
			var spin: float = TAU * float(index) / float(count) + phase * 0.34
			var speed: float = 220.0 + float(index % 4) * 36.0
			spiral.append(_shot(Vector2.RIGHT.rotated(spin), speed, 1.0))
		return _result(spiral, 0.60, 1.02, 0.26)
	if pattern == 1:
		var forked: Array = []
		for angle_variant in [-0.56, -0.28, 0.0, 0.28, 0.56]:
			var angle: float = float(angle_variant)
	...
	var host_result: Dictionary = _result(_radial_shots(12 + stage * 2, 275.0, -phase * 0.26), 0.76, 1.30, 0.18)
...
func _radial_shots(count: int, speed: float, phase: float) -> Array:
	...
	for index in range(safe_count):
		var angle: float = TAU * float(index) / float(safe_count) + phase
...
func _shot(direction: Vector2, speed: float, damage: float) -> Dictionary:
	var normalized: Vector2 = direction
...
func audit_contract() -> Dictionary:
	...
	for boss_id in BOSS_IDS:
		for pattern in range(3):
			var built: Dictionary = build_pattern(boss_id, 1, pattern, Vector2.DOWN, 0.5)
