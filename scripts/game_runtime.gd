...
var player_motion_speed: float = 0.0
var player_shoot_timer: float = 0.0
var player_hurt_timer: float = 0.0
var enemy_serial: int = 0
var active_biome: int = -1
...
func _process(delta: float) -> void:
	...
	var can_measure: bool = state == "run" and player.has("pos")
	if can_measure:
		before = player["pos"]
...
func _sfx(id: String, pitch: float = 1.0, volume_db: float = 0.0, throttle_ms: int = 0) -> void:
...
func _biome_index() -> int:
	var depth: int = rooms_cleared
	if rooms.has(current_room):
		var room: Dictionary = rooms[current_room]
		depth = int(room.get("depth", rooms_cleared))
	return posmod(floori(float(depth) / 2.0), 5)

func _sync_biome_audio(force: bool = false) -> void:
	var biome: int = _biome_index()
...
func fire_player_weapon() -> void:
	...
	var shot_index: int = posmod(selected_lineage, 3) + 1
...
func kill_enemy(index: int) -> void:
	...
	_sfx("boss_phase" if enemy_type == "boss" else "kill", 0.86 if enemy_type == "boss" else randf_range(0.90, 1.12), -2.0, 35)

func damage_player(amount: float) -> void:
	var had_shield: bool = bool(player.get("shield", false)) if not player.is_empty() else false
...
func apply_item(id: String) -> void:
	var before: int = int(player.get("inventory", []).size()) if not player.is_empty() else 0
	super.apply_item(id)
...
func purchase_shop_item(index: int) -> bool:
	...
	var result: bool = super.purchase_shop_item(index)
...
func update_boss(enemy: Dictionary, direction: Vector2, distance: float, delta: float) -> void:
	var stage_before: int = int(enemy.get("boss_stage", 0))
...
func enter_room(coord: Vector2i, movement_direction: Vector2i) -> void:
	...
	var spawn_position: Vector2 = arena.get_center()
...
func check_room_transition() -> void:
	...
	var edge: float = PLAYER_RADIUS + 2.0
...
func _draw_atlas(texture: Texture2D, position: Vector2, frame_size: Vector2, frame: int, row: int, scale: float = 1.0, flip_x: bool = false, modulate: Color = Color.WHITE) -> void:
	if texture == null:
		return
	var source: Rect2 = Rect2(Vector2(float(frame) * frame_size.x, float(row) * frame_size.y), frame_size)
	var signed_scale: Vector2 = Vector2(-scale if flip_x else scale, scale)
	draw_set_transform(position, 0.0, signed_scale)
	draw_texture_rect_region(texture, Rect2(-frame_size * 0.5, frame_size), source, modulate)
...
func _player_fps(row: int) -> float:
	match row:
		1: return 10.0
		2: return 14.0
		3: return 16.0
...
func draw_player() -> void:
	...
	if texture == null:
		super.draw_player()
		return
	var row: int = _player_animation_row()
	var fps: float = _player_fps(row)
	var frame: int = posmod(int(visual_clock * fps), 8)
	...
	if bool(player.get("halo", false)):
		var halo: Vector2 = halo_position()
...
func draw_enemies() -> void:
	for enemy in enemies:
		var type: String = String(enemy.get("type", "feral"))
		var pos: Vector2 = enemy["pos"]
		var texture: Texture2D = null
		var frame_size: Vector2 = Vector2(48.0, 48.0)
		var row: int = 1
		var fps: float = 9.0
		var scale: float = maxf(0.92, float(enemy.get("radius", 17.0)) / 18.0)
		if type == "boss":
		...
		if texture == null:
			super.draw_enemies()
			return
		var frame: int = posmod(int((visual_clock + float(enemy.get("visual_seed", 0)) * 0.17) * fps), 8)
		var flip: bool = player.has("pos") and float(player["pos"].x) < pos.x
		var tint: Color = Color.WHITE if float(enemy.get("flash", 0.0)) <= 0.0 else Color(1.0, 0.86, 0.82, 1.0)
		_draw_atlas(texture, pos, frame_size, frame, row, scale, flip, tint)
		var ratio: float = maxf(0.0, float(enemy["hp"]) / float(enemy["max_hp"]))
		var bar_width: float = float(enemy["radius"]) * 2.1
...
func draw_bullets() -> void:
	...
	for bullet in bullets:
		var owner: String = String(bullet.get("owner", "enemy"))
		var row: int = 5 if bool(bullet.get("critical", false)) else (0 if owner == "player" else 1)
		var frame: int = posmod(int(visual_clock * 18.0 + float(bullet["pos"].x + bullet["pos"].y) * 0.03), 8)
		var scale: float = maxf(0.42, float(bullet.get("radius", 5.0)) / 8.0)
...
func draw_pickups() -> void:
	...
	for pickup in pickups:
		var phase: float = float(pickup.get("phase", 0.0))
		var position: Vector2 = pickup["pos"] + Vector2(0.0, sin(phase * 2.4) * 4.0)
		if String(pickup.get("kind", "")) == "item":
			var row: int = asset_catalog.item_row(String(pickup.get("id", "seraph_lens")))
			var frame: int = posmod(int(visual_clock * 6.0), 4)
			_draw_atlas(relic_texture, position, Vector2(32.0, 32.0), frame, row, 1.15)
		else:
			var effect_row: int = 3 if String(pickup.get("kind", "")) == "heart" else 6
			var effect_frame: int = posmod(int(visual_clock * 12.0), 8)
...
func draw_arena() -> void:
	...
	for y in range(int(arena.position.y), int(arena.end.y), tile_size):
		...
		for x in range(int(arena.position.x), int(arena.end.x), tile_size):
			var tile_index: int = posmod(grid_x * 7 + grid_y * 11 + _biome_index() * 5, 32)
			var source: Rect2 = Rect2(Vector2(float(tile_index % 8) * 32.0, float(floori(float(tile_index) / 8.0)) * 32.0), Vector2(32.0, 32.0))
			var destination: Rect2 = Rect2(Vector2(x, y), Vector2(mini(tile_size, int(arena.end.x) - x), mini(tile_size, int(arena.end.y) - y)))
			draw_texture_rect_region(texture, destination, source, Color.WHITE)
			grid_x += 1
		grid_y += 1
...
func draw_lineage_select() -> void:
	...
	for index in range(lineages.size()):
		...
		if texture != null:
			var frame: int = posmod(int(visual_clock * 5.0 + index), 8)
