...
func draw_pause() -> void:
	...
	var panel: Rect2 = Rect2(Vector2(safe.get_center().x - panel_width * 0.5, safe.get_center().y - 220.0), Vector2(panel_width, 440.0))
	...
	draw_text_centered(prompt, Vector2(panel.get_center().x, panel.end.y - 32.0), 10, Color(tokens["muted"]))
