...
const EngineBootstrapScript: Script = preload("res://scripts/v6/engine_bootstrap.gd")

var production_assets: RefCounted = ProductionAssetRegistryScript.new()
var engine_bootstrap: RefCounted = EngineBootstrapScript.new()
var engine_report: Dictionary = {}
var v6_asset_report: Dictionary = {}
var _v6_sfx_last_ms: Dictionary = {}

func _ready() -> void:
	engine_report = engine_bootstrap.call("configure")
	super._ready()
...
func draw_arena() -> void:
	...
	if background != null:
		...
		push_error("Missing v0.6 biome background: %s" % biome_id)
		draw_rect(arena, Color8(255, 0, 255))
	...
	if tile_texture != null:
		...
		for y in range(int(arena.position.y), int(arena.end.y), 32):
			...
			for x in range(int(arena.position.x), int(arena.end.x), 32):
				var tile_index := posmod(tile_column * 7 + tile_row * 11 + biome_index * 5, 32)
		...
		push_error("Missing v0.6 biome tiles: %s" % biome_id)
	...
	if props != null:
		...
		push_error("Missing v0.6 biome props: %s" % biome_id)
...
func draw_bullets() -> void:
	...
	if atlas == null:
		push_error("Missing v0.6 projectile atlas")
		return
	var frame := posmod(int(visual_clock * 18.0), 8)
	for bullet in bullets:
		var atlas_row := 5
		if String(bullet["owner"]) == "player":
			atlas_row = clampi(selected_lineage, 0, 4)
		var source := Rect2(Vector2(float(frame * 16), float(atlas_row * 16)), Vector2(16, 16))
...
func draw_pickups() -> void:
	...
	if pickup_atlas == null or relic_atlas == null:
		push_error("Missing v0.6 pickup or relic atlas")
		return
	for pickup in pickups:
		...
		if kind == "relic":
			...
			var relic_source := Rect2(Vector2(float(relic_number % 12) * 32.0, float(relic_number / 12) * 32.0), Vector2(32, 32))
			draw_texture_rect_region(relic_atlas, Rect2(pos - Vector2(18, 18), Vector2(36, 36)), relic_source)
		else:
			...
			match kind:
				...
				_: cell = Vector2i(3, 0)
			var pickup_source := Rect2(Vector2(cell * 48), Vector2(48, 48))
			draw_texture_rect_region(pickup_atlas, Rect2(pos - Vector2(20, 20), Vector2(40, 40)), pickup_source)
...
func draw_touch_controls() -> void:
	...
	if pause_texture != null:
		var pause_rect := Rect2(safe_rect().end - Vector2(84, safe_rect().size.y - 12), Vector2(58, 58))
		draw_texture_rect(pause_texture, pause_rect, false, Color(1, 1, 1, 0.88))
...
func draw_title() -> void:
	...
	var patch := Rect2(Vector2(safe.get_center().x - 260.0, hero_y + 45.0), Vector2(520.0, 28.0))
	draw_rect(patch, Color8(5, 10, 11))
	var running_version := String(engine_report.get("running", "unknown"))
	draw_text_centered("v%s  •  GODOT %s  •  PRODUCTION ART ACTIVE" % [V6_VERSION, running_version], Vector2(safe.get_center().x, hero_y + 63.0), 11, Color8(126, 182, 145))

func draw_select() -> void:
	super.draw_select()
	var safe := safe_rect()
	draw_text_centered("PRODUCTION SHEETS // 8 DIRECTIONS × 8 FRAMES", Vector2(safe.get_center().x, safe.end.y - 10.0), 10, Color8(130, 165, 150))
...
func audit_v6_readiness() -> Dictionary:
	...
	else: errors.append("Production index version mismatch")
	checks += 1
	...
	var asset_contract: Dictionary = production_assets.call("validate_contract")
	checks += 1
	...
	else: errors.append("Project version is not v0.6.0")
	checks += 1
...
func get_v6_diagnostics() -> Dictionary:
	return {
		"engine": engine_report.duplicate(true),
		"assets": v6_asset_report.duplicate(true),
